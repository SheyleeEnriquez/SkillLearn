...
function p_generar_factura_xml() {
    ...
    var nombreFacturacion = document.getElementById("razon_social_comprador").value;
    var identidadFacturacion = document.getElementById("identificacion_comprador").value;
    var direccionFacturacion = document.getElementById("dirección_comprador").value;
    var tipo_documento_factura = document.getElementById("tipo_documento_factura").value;
    var establecimiento_factura = document.getElementById("establecimiento_factura").value;
    var punto_emision_factura = document.getElementById("punto_emision_factura").value;
    var secuencial_factura = document.getElementById("secuencial_factura").value;
    var direccion_matriz_universidad = document.getElementById("direccion_matriz_universidad").value;
    var direccion_establecimiento = document.getElementById("direccion_establecimiento").value;
    var fecha_venta = document.getElementById("fecha_venta").value;
    var llevar_contabilidad = document.getElementById("llevar_contabilidad").value;
    var tipo_identificacion_comprador = document.getElementById("tipo_identificacion_comprador").value;
    var total_sin_impuestos = document.getElementById("total_sin_impuestos").value;
    var total_descuento = document.getElementById("total_descuento").value;
    var tabla = document.getElementById('tablaItems');
    var inputsTablaItems = tabla.querySelectorAll('input, select');
    var filasDinamicas = tabla.querySelectorAll('tr[data-indice]');

    var subtotal_grava = document.getElementById("subtotal_grava").value;
    var valor_grava = document.getElementById("valor_grava").value;
    var subtotal_0 = document.getElementById("subtotal_0").value;
    var valor_total = document.getElementById("valor_total").value;
    var iva_decimal = document.getElementById("iva_decimal").value;
    var identificacion_comprador = document.getElementById("identificacion_comprador").value;
    ...
    var correo_facturacion = document.getElementById("correo_facturacion").value;
    var maestria_maestreante = document.getElementById("maestria_maestreante").value;
    var cohorte_maestreante = document.getElementById("cohorte_maestreante").value;
                ...
                campoAdicional: [
                    {
                        ...
                        __text: "Nombre: " + nombreFacturacion + " Cédula o Ruc: " + identificacion_comprador + " Correo: " + correo_facturacion
                    }
    ...
    estructuraFactura[tipoComprobante].infoTributaria.codDoc = tipo_documento_factura.toString().padStart(2, '0'); //tipo de comprobante
    ...
    document.getElementById('establecimiento').value = establecimiento_factura;
    document.getElementById('punto_emision').value = punto_emision_factura;
    document.getElementById('secuencial').value = secuencial_factura;

    estructuraFactura[tipoComprobante].infoFactura.fechaEmision = moment().format('DD/MM/YYYY');
    estructuraFactura[tipoComprobante].infoFactura.dirEstablecimiento = direccion_establecimiento;
    //estructuraFactura[tipoComprobante].infoFactura.contribuyenteEspecial = '5368';
    estructuraFactura[tipoComprobante].infoFactura.obligadoContabilidad = llevar_contabilidad ?? 'NO';
    estructuraFactura[tipoComprobante].infoFactura.tipoIdentificacionComprador = tipo_identificacion_comprador;
    //estructuraFactura[tipoComprobante].infoFactura.guiaRemision = '001-100-000000001';
    estructuraFactura[tipoComprobante].infoFactura.razonSocialComprador = nombreFacturacion;
    estructuraFactura[tipoComprobante].infoFactura.identificacionComprador = identidadFacturacion;
    estructuraFactura[tipoComprobante].infoFactura.direccionComprador = direccionFacturacion;
    estructuraFactura[tipoComprobante].infoFactura.totalSinImpuestos = parseFloat(total_sin_impuestos).toFixed(2);
    estructuraFactura[tipoComprobante].infoFactura.totalDescuento = parseFloat(total_descuento).toFixed(2);

    estructuraFactura[tipoComprobante].infoFactura.totalConImpuestos.totalImpuesto[0].descuentoAdicional = 0.00;
    estructuraFactura[tipoComprobante].infoFactura.totalConImpuestos.totalImpuesto[0].baseImponible = parseFloat(subtotal_0).toFixed(2);
    estructuraFactura[tipoComprobante].infoFactura.totalConImpuestos.totalImpuesto[0].valor = parseFloat(valor_grava).toFixed(2);

    estructuraFactura[tipoComprobante].infoFactura.pagos.pago[0].formaPago = forma_pago;
    estructuraFactura[tipoComprobante].infoFactura.pagos.pago[0].total = parseFloat(valor_pago).toFixed(2);
    estructuraFactura[tipoComprobante].infoFactura.pagos.pago[0].plazo = plazo;
    estructuraFactura[tipoComprobante].infoFactura.pagos.pago[0].unidadTiempo = unidad_tiempo;


    //estructuraFactura[tipoComprobante].infoFactura.totalConImpuestos.totalImpuesto[1].descuentoAdicional = 0.00;
    //estructuraFactura[tipoComprobante].infoFactura.totalConImpuestos.totalImpuesto[1].baseImponible = parseFloat(subtotal_0).toFixed(2);
    //estructuraFactura[tipoComprobante].infoFactura.totalConImpuestos.totalImpuesto[1].valor = 0.00;

    estructuraFactura[tipoComprobante].infoFactura.propina = '0.00';
    estructuraFactura[tipoComprobante].infoFactura.importeTotal = parseFloat(valor_total).toFixed(2);
    ...
    inputsTablaItems.forEach(function (input) {
        var nombre = input.getAttribute('name');
        var valor = input.value;

        switch (nombre) {
            case 'codigo_valor_venta':
                estructuraFactura[tipoComprobante].detalles.detalle[0].codigoPrincipal = valor;
                estructuraFactura[tipoComprobante].detalles.detalle[0].codigoAuxiliar = valor;
                break;
            case 'descripcion_venta':
                estructuraFactura[tipoComprobante].detalles.detalle[0].descripcion = valor;
                break;
            case 'cantidad_creditos_venta':
                estructuraFactura[tipoComprobante].detalles.detalle[0].cantidad = valor;
                break;
            case 'costo_unitario':
                estructuraFactura[tipoComprobante].detalles.detalle[0].precioUnitario = parseFloat(valor).toFixed(2);
                break;
            case 'total_descuento':
                estructuraFactura[tipoComprobante].detalles.detalle[0].descuento = parseFloat(valor).toFixed(2);
                break;
            case 'subtotal':
                estructuraFactura[tipoComprobante].detalles.detalle[0].precioTotalSinImpuesto = parseFloat(valor).toFixed(2);
                estructuraFactura[tipoComprobante].detalles.detalle[0].impuestos.impuesto[0].baseImponible = parseFloat(valor).toFixed(2);
                estructuraFactura[tipoComprobante].detalles.detalle[0].impuestos.impuesto[0].valor = 0 * parseFloat(iva_decimal).toFixed(2);
                break;
            default:
                break;
        }
    ...
    filasDinamicas.forEach(function (fila) {
        var indice = fila.getAttribute('data-indice');
        var inputsFila = fila.querySelectorAll('input, select');

        inputsFila.forEach(function (input) {
            var nombre = input.getAttribute('name');
            var valor = input.value;

            // Asignar los valores de los campos de la fila dinámica a la estructura de datos
            switch (nombre) {
                case 'filas[][cantidad_creditos]':
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].cantidad = valor;
                    break;
                case 'filas[][codigo]':
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].codigoPrincipal = valor;
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].codigoAuxiliar = valor;
                    break;
                case 'filas[][descripcion]':
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].descripcion = valor;
                    break;
                case 'filas[][costo_unitario]':
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].precioUnitario = parseFloat(valor).toFixed(2);
                    break;
                case 'filas[][total_descuento]':
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].descuento = parseFloat(valor).toFixed(2);
                    break;
                case 'filas[][subtotal]':
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].precioTotalSinImpuesto = parseFloat(valor).toFixed(2);
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].impuestos.impuesto[0].baseImponible = parseFloat(valor).toFixed(2);
                    estructuraFactura[tipoComprobante].detalles.detalle[indice].impuestos.impuesto[0].valor = parseFloat(valor).toFixed(2) * parseFloat(iva_decimal).toFixed(2);
                    break;
                default:
                    break;
            }
    ...
    //estructuraFactura[tipoComprobante].detalles.detalle[0].detallesAdicionales.detAdicional[0]._nombre = 'ABCD';
    //estructuraFactura[tipoComprobante].detalles.detalle[0].detallesAdicionales.detAdicional[0]._valor = 'EFGH';

    //estructuraFactura[tipoComprobante].detalles.detalle[0].impuestos.impuesto[1].baseImponible = '64.94';
    //estructuraFactura[tipoComprobante].detalles.detalle[0].impuestos.impuesto[1].valor = '3.25';

    //estructuraFactura[tipoComprobante].detalles.detalle[0].impuestos.impuesto[2].baseImponible = '64.94';
    //estructuraFactura[tipoComprobante].detalles.detalle[0].impuestos.impuesto[2].valor = '3.25';
...
function saveFile_noui(xmlData, fileName, firmaFunction) {

    firmaFunction(xmlData, fileName);

}

function p_firmar_factura(xmlData, fileName) {

    var fileInput = xmlData;

    if (fileInput) {

        var xmlContent = xmlData;

        p_generar_xades_bes(xmlContent, function (factura_firmada, claveAcceso) {
            //console.log('factura_firmada:', factura_firmada);
            //console.log('base 64 de factura firmada: ', forge.util.encode64(factura_firmada));
            var factura = forge.util.encode64(factura_firmada);
            var Clave_Acceso = claveAcceso;
            var xml = xmlContent;
            var Factura_Firmada = factura_firmada;
            var establecimiento = document.getElementById('establecimiento_factura').value;
            var puntoEmision = document.getElementById('punto_emision_factura').value;
            var codigo = document.getElementById('secuencial_factura').value;

            const blob = new Blob([Factura_Firmada], { type: 'text/xml' });

            const downloadLink = document.createElement('a');
            downloadLink.href = URL.createObjectURL(blob);
            downloadLink.download = Clave_Acceso;
            document.body.appendChild(downloadLink);
            downloadLink.click();

            document.getElementById('xmlContent').value = factura_firmada;
            document.getElementById('claveAcceso').value = Clave_Acceso;
            document.getElementById('establecimiento').value = establecimiento;
            document.getElementById('punto_emision').value = puntoEmision;
            document.getElementById('secuencial').value = codigo;
            form.submit();
        });
    } else {
